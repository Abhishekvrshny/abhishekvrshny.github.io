...
### Tail Latencies
Latencies are generally plotted and monitored at percentiles of p90, p95, p99 etc. But as you move towards the higher end of the spectrum, the tail latencies keep increasing. [The Tail at Scale](https://cacm.acm.org/magazines/2013/2/160173-the-tail-at-scale/fulltext) paper by Google, in fact, talks in detail about this and also mentions various techniques to counter it. One of the techniques mentioned is **Request Hedging**. This post in on tail latencies, and how request hedging was used to curtail tail latencies in one of the high scale services at [Razorpay](https://razorpay.com/tech/). 

This service in picture is the notifications' platform. Lets talk about it a little bit before jumping into the problem and its solution.

### The Notifications' Service
The notifications' service in Razorpay is a high througput service which receives requests to send out various types of notifications like webhooks, SMS, emails etc. at a peak rate of approximately 2000 requests/sec.
...
### The Solution
While the Google paper talks about Hedged Requests primarily in context of reads, we used it in the write flow and piggybacked on the database and the cron setup, which was already in place, to write the request to the database if SQS push doesn't succeed within the defined timeout period. One of the drawbacks of this approach is that it can lead to duplicate deliveries, but that was acceptable as we anyway promised at [least once delivery semantics](https://razorpay.com/docs/webhooks/#idempotency). 
...
3. It can even lead to connections never getting establised even in case of some minor degradation in any of the aforementioned phases.
